# mbe_demo_pitl.jl
#
# This script runs the control law on the target processor.
...
# Load the YAML file and change the model name to use the PITL version.
# Also, change the log name.
yaml = YAML.load_file(yaml_file)
yaml["vehicles"][1]["computers"][1]["software"][1]["model"] = "ReducedEffortUnderactuatedControllerPITL"
yaml["sim"]["log_file"] = "out/underactuated-pitl.h5"

# The parameters are now stored under constants/parameters, so move them. Also,
# set up the UDP parameters here.
yaml["vehicles"][1]["computers"][1]["software"][1]["constants"] = Dict(
    "parameters"=>yaml["vehicles"][1]["computers"][1]["software"][1]["constants"],
    "conn"=>Dict(
        "target_ip"=>ip"192.168.1.195",
        "send_port"=>2000,
        "receive_port"=>2001
    )
);
